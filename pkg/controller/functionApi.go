...
func RegisterFunctionRoute(ws *restful.WebService) {
	...
	ws.Route(
		ws.GET("/v2/functions/logs/{function}").
			Doc("Get pods running the function").
			Metadata(restfulspec.KeyOpenAPITags, tags).
			To(func(req *restful.Request, resp *restful.Response) {
				resp.ResponseWriter.WriteHeader(http.StatusOK)
			}).
			...
			Returns(http.StatusOK, "Function Logs", fv1.Function{}))
}
...
func (a *API) FunctionPodLogs(w http.ResponseWriter, r *http.Request) {
	...
	if f.Spec.Environment.Namespace != metav1.NamespaceDefault {
		// jingtao add is the namespace of environment not function
	...
	if len(pods) <= 0 {
		w.Write(createErrorResponseJsonString("no active pods found"))
		return
	}
	...
	err = getContainerLog(a.kubernetesClient, w, f, &pods[0], podLogOpts)
	if err != nil {
...
func getContainerLog(kubernetesClient *kubernetes.Clientset, w http.ResponseWriter, fn *fv1.Function, pod *apiv1.Pod, podLogOpts apiv1.PodLogOptions) error {
